...
レビューを受けたり同期のプルリクを眺めている内に、それぞれの階層の責務を分けること、モジュールのコールが増えてもパフォーマンスが悪くならないように意識する癖が身についたと感じています。サーバ側はPerlを使ってコーディングを行っていますが、構文に慣れれば冗長なトークンも少ないので、ななめ読みで抽象的に捉えやすく感じます。そのような状況なので、特にサーバ側のアーキテクチャを意識することも多くなりました。  
学生の頃はフロントエンド技術ばかりを扱っていましたが、仕事をしている内にサーバ側やインフラに興味が傾いてきました。
